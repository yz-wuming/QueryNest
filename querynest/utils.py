"""
Utility functions for QueryNest
...
def _html_table_to_text(html: str) -> str:
    """Flatten an HTML table into plain-text rows for search indexing."""
    import re
...
def table_to_plain_text(item: Dict[str, Any]) -> str:
    """Extract searchable plain text from a table content-list item."""
    raw = get_table_body(item)
    if isinstance(raw, str) and "<table" in raw.lower():
        return _html_table_to_text(raw)
    return format_table_body(raw)
...
def separate_content(
    ...
    for index, item in enumerate(content_list):
        ...
        if content_type == "text":
            ...
            # 表格正文并入全文，保证表格内容可被文本检索命中
            table_text = table_to_plain_text(item)
            if table_text.strip():
