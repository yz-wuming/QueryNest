"""
Prompt language management for QueryNest.
...
Usage (process-global switch)::
    ...
    from querynest.prompt_manager import set_prompt_language, get_prompt_language
...
def _lazy_load_language(lang: str) -> Dict[str, Any]:
    ...
    if lang == "zh":
        from querynest.prompts_zh import PROMPTS_ZH
...
def register_prompt_language(language_code: str, prompts: Dict[str, Any]) -> None:
    ...
    Args:
        ...
        prompts: Dictionary of prompt templates, using the same keys as
                 :data:`querynest.prompt.PROMPTS`.

    Example::

        from querynest.prompt_manager import register_prompt_language
