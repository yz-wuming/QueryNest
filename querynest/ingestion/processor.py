"""
Document processing functionality for QueryNest
...
from querynest.base import DocStatus
from querynest.ingestion.parser import MineruParser, MineruExecutionError, get_parser
from querynest.utils import (
    separate_content,
    insert_text_content,
...
class ProcessorMixin:
    """ProcessorMixin class containing document processing functionality for QueryNest"""
    ...
    def _apply_chunk_template(
        ...
        """
        from querynest.prompt import PROMPTS
    ...
    async def process_document_complete(
        ...
        **kwargs,
        ...
        try:
            ...
            # PDF Index Bridge：把解析产物与最终 doc_id 保存为实例状态，供上层
            # （engine.ingest）读取以持久化规范正文来源，而无需改动本方法返回值。
            self._last_parse_content_list = content_list
            self._last_parse_doc_id = doc_id
            self._last_parse_file_name = file_name
            ...
            if pipeline_status_lock and pipeline_status:
                try:
                    async with pipeline_status_lock:
                        ...
                        error_msg = (
                            f"QueryNest processing failed for {file_name}: {str(e)}"
                        )
                ...
                try:
                    async with pipeline_status_lock:
                        ...
                        pipeline_status["latest_message"] = (
                            f"QueryNest processing completed for {file_name}"
                        )
                        pipeline_status["history_messages"].append(
                            f"QueryNest processing completed for {file_name}"
                        )
