...
- The custom parser registry is primarily intended for Python usage, where your
  ...
  before invoking QueryNest APIs.
- The standalone CLI (``python -m querynest.ingestion.parser`` or the installed console
...
from querynest.asset_urls import attach_public_media_urls
...
_MINERU_KNOWN_FAILURES: Tuple[Tuple[str, str], ...] = (
    (
        ...
        'natively without going through MinerU: QueryNestConfig(parser="docling").',
    ),
)
...
class MineruParser(Parser):
    ...
    # ------------------------------------------------------------------ #
    # Direct .docx parsing (python-docx): 保留段落顺序，避免 MinerU OCR 乱序
    # ------------------------------------------------------------------ #
    _IMAGE_EXT_BY_TYPE = {
        "image/png": ".png",
        "image/jpeg": ".jpg",
    ...
    @classmethod
    def _iter_docx_block_items(cls, parent) -> Iterator[Any]:
    ...
    @classmethod
    def _iter_docx_paragraph_image_rids(cls, paragraph) -> Iterator[str]:
        """Yield rIds of blips embedded in a docx paragraph."""
        from docx.oxml.ns import qn
    ...
    @classmethod
    def _save_docx_image_part(cls, doc, r_id: str, img_dir: Path, idx: int) -> Optional[str]:
        """Save an embedded image part to disk; return its path or None."""
        part = doc.part.related_parts.get(r_id)
        if part is None or not getattr(part, "blob", None):
            return None
        content_type = getattr(part, "content_type", "") or ""
        ext = cls._IMAGE_EXT_BY_TYPE.get(content_type, ".png")
        path = img_dir / f"image_{idx:03d}{ext}"
        with open(path, "wb") as f:
            f.write(part.blob)
        return str(path)

    def _parse_docx_with_python_docx(
        self, docx_path: Union[str, Path], output_dir: Optional[str] = None
    ) -> Optional[List[Dict[str, Any]]]:
        """Parse a .docx directly with python-docx (text/table/embedded images).

        Returns None when python-docx is unavailable so the caller can fall back.
        """
        try:
            ...
            self.logger.warning("python-docx 未安装，.docx 回退到 MinerU 转换解析")
            return None

        doc = Document(str(docx_path))
        img_root = Path(output_dir) if output_dir else Path(docx_path).parent
        img_dir = img_root / f"{Path(docx_path).stem}_docx_images"
        ...
        for block in self._iter_docx_block_items(doc):
            if isinstance(block, Paragraph):
                for r_id in self._iter_docx_paragraph_image_rids(block):
                    saved = self._save_docx_image_part(doc, r_id, img_dir, img_idx)
                    if saved:
                        content_list.append({"type": "image", "img_path": saved})
                        img_idx += 1
        ...
        self.logger.info(
            f"Direct docx parse: {len(content_list)} blocks from {Path(docx_path).name}"
        )
        return content_list
    ...
    def _prepare_mineru_paths(
        ...
        temp_dir = Path(tempfile.mkdtemp(prefix="querynest_mineru_"))
    ...
    def parse_office_doc(
        ...
        **kwargs,
        ...
        """
        doc_path = Path(doc_path)
        # .docx 优先直接用 python-docx 提取，保留段落顺序（避免 MinerU OCR 乱序）
        if doc_path.suffix.lower() == ".docx":
            try:
                content_list = self._parse_docx_with_python_docx(doc_path, output_dir)
                if content_list is not None:
                    return content_list
            except Exception as e:  # noqa: BLE001
                self.logger.warning(f"Direct docx parse failed ({e}), falling back to MinerU")
        try:
...
def register_parser(name: str, parser_class: type) -> None:
    """Register a custom parser class for use with QueryNest.
    ...
    Example::
        ...
        from querynest.ingestion.parser import Parser, register_parser
...
SUPPORTED_PARSERS = ("mineru", "docling", "paddleocr", "lite")
...
def get_parser(parser_type: str) -> Parser:
    ...
    if parser_name == "lite":
        # QueryNest 轻量纯文本解析器（无重型依赖）；延迟 import 避免循环依赖
        from querynest.ingestion.lite import LiteTextParser

        return LiteTextParser()
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Python process are also accepted when you integrate QueryNest "
