"""
Query functionality for QueryNest
...
class QueryMixin:
    """QueryMixin class containing query functionality for QueryNest"""
        ...
        **kwargs,
        ...
        if not hasattr(self, "vision_model_func") or not self.vision_model_func:
            raise ValueError(
                ...
                "Please provide a vision model function when initializing QueryNest."
            )
    ...
    async def _call_vlm_with_multimodal_content(self, messages: List[Dict]) -> str:
        ...
        try:
            ...
            if isinstance(content, str):
                # Pure text mode
                result = self.vision_model_func(content, system_prompt=system_prompt)
            else:
                # Multimodal mode - pass complete messages directly to VLM
                result = self.vision_model_func(
                    "",  # Empty prompt since we're using messages format
                    messages=messages,
                )

            # vision_model_func 可能是同步或异步实现，统一兼容
