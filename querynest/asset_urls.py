...
def attach_public_media_urls(item: dict) -> None:
    ...
    """
    base = _get_asset_env("PUBLIC_ASSET_BASE_URL").strip()
    strip = _get_asset_env("PUBLIC_ASSET_STRIP_PREFIX").strip()
    ...
    if base and not strip:
        if "base_only" not in _MISCONFIG_WARNED:
            logger.warning(
                "QUERYNEST_PUBLIC_ASSET_BASE_URL is set but "
                "QUERYNEST_PUBLIC_ASSET_STRIP_PREFIX is not; "
    ...
    if strip and not base:
        if "strip_only" not in _MISCONFIG_WARNED:
            logger.warning(
                "QUERYNEST_PUBLIC_ASSET_STRIP_PREFIX is set but "
                "QUERYNEST_PUBLIC_ASSET_BASE_URL is not; "
                "skipping public URL attachment."
            )
