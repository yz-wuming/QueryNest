...
| 测试体系 | `tests/` | 9 个新测试文件共 82 个用例（详见项目报告） |
| 文档 | `README.md`, `REFACTOR_ANALYSIS.md`, `env.example`, `pyproject.toml`, `setup.py` | 品牌化重写 |
...
- 默认测试套件（`pytest tests/`）：**82 passed, 0 failed**。
- 覆盖：配置 / 文档解析注册表 / 多模态（公式提取）/ 检索（Hybrid/BM25/Reranker/Context）/
  Query（Analyzer/Rewrite）/ Citation / Document CRUD / Evaluation / API。
- `tests/legacy/` 保留自 RAG-Anything 继承的旧测试（依赖可选重型组件：MinerU/PaddleOCR/torch 与
  旧 `raganything` 模块路径），不在默认套件中运行。
